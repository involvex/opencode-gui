...
export class OpenCodeViewProvider implements vscode.WebviewViewProvider {
	...
	public resolveWebviewView(
		...
		webviewView.webview.options = {
			...
			portMapping: devServerConfig
				? [
						{
							webviewPort: devServerConfig.port,
							extensionHostPort: devServerConfig.port,
						},
					]
				: [],
		}
	...
	private _getDevServerConfig(): DevServerConfig | null {
		...
		try {
			...
			getLogger().warn(
				'[ViewProvider] Invalid OPENCODE_DEV_SERVER_URL; falling back to bundled webview',
			)
			return null
		}
	}
	...
	private _getHtmlForWebview(
		...
		devServerConfig: DevServerConfig | null,
	) {
		if (devServerConfig) {
			return `<!DOCTYPE html>
        <html lang="en">
