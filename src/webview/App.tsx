...
function App() {
	...
	const [draftContents, setDraftContents] = createSignal<Map<string, any>>(
		new Map(),
	) // TipTap JSON content
	const [sessionAgents, setSessionAgents] = createSignal<Map<string, string>>(
		new Map(),
	)
	const [selectionAttachmentsBySession, setSelectionAttachmentsBySession] =
		createSignal<Map<string, SelectionAttachment[]>>(new Map())
	...
	const [pendingMentionInsertions, setPendingMentionInsertions] = createSignal<
	...
	const getSdkErrorMessage = (error: unknown): string => {
		...
		if (nestedError && typeof nestedError === 'object') {
			...
			if (nestedData && typeof nestedData === 'object') {
				const nestedDataMessage = (nestedData as Record<string, unknown>)
					.message
				if (
					typeof nestedDataMessage === 'string' &&
					nestedDataMessage.length > 0
				) {
	...
	const isDefaultTitle = (title: string) =>
		/^(New session|Child session) - \d{4}-\d{2}-\d{2}T/.test(title)
	...
	const selectedAgent = () => {
		...
		return sessionId
			? sessionAgents().get(sessionId) || defaultAgent()
			: defaultAgent()
	}
	...
	const selectionAttachments = () =>
		selectionAttachmentsBySession().get(sessionKey()) || []
	const setSelectionAttachmentsForKey = (
		key: string,
		value:
			| SelectionAttachment[]
			| ((prev: SelectionAttachment[]) => SelectionAttachment[]),
	) => {
	...
	const openFileFromMention = (filePath: string) => {
		...
		if (!workspaceRoot) {
			logger.error('Cannot open mention: workspace root unavailable', {
				filePath,
			})
			return
		}
	...
	const insertMentionFromHostSelection = (
		request: FileMentionInsertRequest,
	): boolean => {
		if (!editorMethods) {
			return false
		}
		...
		try {
			...
			if (!existingMentions.has(requestKey)) {
				editorMethods.insertFileMention(
					request.filePath,
					request.startLine,
					request.endLine,
				)
	...
	const handleSubmit = async () => {
		...
		if (editorMethods) {
			try {
				...
				if (!workspaceRoot) {
					throw new Error(
						'workspace root unavailable while extracting mentions',
					)
				}
				...
				const mentionAttachments: SelectionAttachment[] = mentionedFiles
					.map(mentionReference => {
						...
						return {
							...
							fileUrl: buildWorkspaceFileUrl(
								workspaceRoot,
								parsedMention.filePath,
							),
					...
					.filter(
						(attachment): attachment is SelectionAttachment =>
							attachment !== null,
					)
