...
export function FileMentionDropdown(
	props: FileMentionDropdownProps & {
		ref?: (ref: FileMentionDropdownRef) => void
	},
) {
	...
	return (
		...
		>
			<Show
				...
				fallback={
					<div class="file-mention-dropdown__empty">No files found</div>
				}
			>
					...
					{(item, index) => {
						...
						return (
							<div
								class={`file-mention-dropdown__item ${
									index() === localSelectedIndex()
										? 'file-mention-dropdown__item--selected'
										: ''
								}`}
								data-index={index()}
							...
							>
								...
								{directory && (
									<span class="file-mention-dropdown__path">{directory}</span>
								)}
							</div>
						)
					}}
