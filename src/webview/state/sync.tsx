...
function createSync() {
	...
	const [currentSessionId, setCurrentSessionIdInternal] = createSignal<
		string | null
	>(null)
	...
	const sessions = createMemo(() => store.sessions.slice(), undefined, {
		equals: false,
	})
	const agents = createMemo(() => store.agents.slice(), undefined, {
		equals: false,
	})
	...
	function handleEvent(event: Event) {
		...
		if (store.status.status === 'bootstrapping') {
			logger.debug('Event queued during bootstrap', {
				type: event.type,
				queueLength: eventQueue.length,
			})
			return
	...
	const sessionStatus = (sessionId: string) =>
		store.sessionStatus[sessionId] ?? null
