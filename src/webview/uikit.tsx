...
import {
	FileMentionDropdown,
	type FileItem,
} from './components/FileMentionDropdown'
...
+  return user.email && user.password;
						...
						diagnostics: {
							'/src/auth/validators.ts': [
								...
								{
									severity: 2,
									message: "Variable 'user' is declared but never used",
								},
							],
						},
					},
...
function UIKit() {
	...
	const toggleError = () => {
		if (errorMessage()) {
			...
			setErrorMessage(
				'This is a test error message! Something went wrong with the service connection.',
			)
		}
	}
	...
	const controlButtons = [
		{
			label: () => (isThinking() ? 'Stop Thinking' : 'Start Thinking'),
			onClick: toggleThinking,
		},
		{label: () => 'Clear Messages', onClick: clearMessages},
		{label: () => 'Load Fake Messages', onClick: loadFakeMessages},
		{
			label: () => `Context % (${contextInfo().percentage.toFixed(0)}%)`,
			onClick: cycleContextPercentage,
		},
		{
			label: () => (showQueuedMessages() ? 'Hide Queue' : 'Show Queue'),
			onClick: toggleQueuedMessages,
		},
		{
			label: () => (errorMessage() ? 'Hide Error' : 'Show Error'),
			onClick: toggleError,
		},
		{
			label: () => (showTiptapTest() ? 'Hide Tiptap' : 'Show Tiptap'),
			onClick: () => setShowTiptapTest(!showTiptapTest()),
		},
		{
			label: () => (showDropdownTest() ? 'Hide Dropdown' : 'Show Dropdown'),
			onClick: () => setShowDropdownTest(!showDropdownTest()),
		},
	]
