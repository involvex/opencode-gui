...
function normalizeLineRange(
	startLine?: number,
	endLine?: number,
): Pick<FileMentionReference, 'startLine' | 'endLine'> {
...
export function encodeFileMentionReference(
	reference: FileMentionReference,
): string {
...
export function formatFileMentionLabel(
	reference: FileMentionReference,
): string {
	const filePath = reference.filePath.trim()
	const normalized = normalizeLineRange(reference.startLine, reference.endLine)
