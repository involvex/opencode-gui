	...
	return {
		...
		render: () => {
			...
			const getReferenceRect = (
				props: Parameters<
					NonNullable<
						ReturnType<NonNullable<SuggestionOptions['render']>>['onStart']
					>
				>[0],
			) => {
			...
			const getDropdownPosition = (
				...
				const dropdownWidth =
					dropdownElement?.offsetWidth ?? DROPDOWN_FALLBACK_WIDTH
				const dropdownHeight =
					dropdownElement?.offsetHeight ?? DROPDOWN_FALLBACK_HEIGHT
				...
				left = Math.min(
					Math.max(left, DROPDOWN_PADDING),
					Math.max(DROPDOWN_PADDING, maxLeft),
				)
			...
			return {
				onStart: props => {
					...
					currentPosition = getDropdownPosition(
						props,
						dropdownRef?.getElement(),
					)
