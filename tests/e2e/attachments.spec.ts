...
test.describe('Attachment/Selection Parity', () => {
	test('should display attachment chip when editor-selection message received', async ({
		openWebview,
	}) => {
		...
		await expect(page.getByText('example.ts L10-25')).toBeVisible({
			timeout: 5000,
		})
	})

	test('should display single-line selection correctly', async ({
		openWebview,
	}) => {
		const page = await openWebview()
	...
	test('should display whole file attachment without line numbers', async ({
		openWebview,
	}) => {
		const page = await openWebview()
	...
	test('should remove attachment when clicking remove button', async ({
		openWebview,
	}) => {
		const page = await openWebview()
		...
		const removeButton = page
			.locator('[data-testid="remove-attachment"]')
			.or(page.getByRole('button', {name: /remove|delete|×|x/i}).first())
	...
	test('should not duplicate identical attachments', async ({openWebview}) => {
		...
		// Wait for chip to appear
		await expect(page.getByText('duplicate.ts L5-15')).toBeVisible({
			timeout: 5000,
		})
	...
	test('should send attachment with prompt and clear attachments', async ({
		...
		// Wait for chip to appear
		await expect(page.getByText('package.json L1-5')).toBeVisible({
			timeout: 5000,
		})
		...
		await expect(page.getByText('package.json L1-5')).not.toBeVisible({
			timeout: 10000,
		})
		...
		).toContainText('What is this file?', {timeout: 10000})
	})
	...
	test('should normalize reversed line selection', async ({openWebview}) => {
		...
		await expect(page.getByText('reversed.ts L30-50')).toBeVisible({
			timeout: 5000,
		})
	})
})
