...
test.describe('Outbox and Idempotent Sends', () => {
	test('should send message and show user message content', async ({
		openWebview,
	}) => {
		...
		).toContainText('Hello from outbox test', {timeout: 10000})
	...
	test('should show thinking state while processing', async ({openWebview}) => {
		...
		await expect(page.getByRole('article', {name: 'user message'})).toBeVisible(
			{timeout: 10000},
		)
