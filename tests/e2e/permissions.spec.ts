...
test.describe.skip('Permissions', () => {
	...
	test('should show permission card when tool needs approval', async ({
		openWebview,
	}) => {
		...
		await textarea.fill(
			"Create a new file called test-permission.txt with the content 'hello world'",
		)
		...
		const permissionGroup = page.getByRole('group', {
	...
	test('should show inline permission for external directory', async ({
		openWebview,
	}) => {
		const page = await openWebview({opencodeConfig: restrictiveConfig})
		...
		const textarea = page.getByRole('textbox', {name: 'Message input'})
		await textarea.fill(
			"Edit the file /tmp/test-external.txt and add the line 'hello world'",
		)
		...
		await expect(page.locator('.permission-prompt__message')).toContainText(
			/allow access|tmp/i,
		)
