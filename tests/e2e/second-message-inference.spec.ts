...
test.describe('Second Message Inference Bug', () => {
	...
	test('should track POST requests for sequential messages', async ({
		...
		postRequests.forEach((req, i) => {
			try {
				...
				console.log(
					`  [${i + 1}] messageID: ${parsed.messageID}, parts: ${parsed.parts?.length}`,
				)
			} catch {
	...
	test('should verify messageID format in requests', async ({
		...
		await page.route('**/session/*/message', async route => {
			if (route.request().method() === 'POST') {
				try {
					...
					const hasMessageID =
						'messageID' in body && body.messageID !== undefined
					requestBodies.push({
						messageID: body.messageID,
						hasMessageID,
					})
					console.log(
						`[test] Captured request: messageID=${body.messageID}, hasMessageID=${hasMessageID}`,
					)
				} catch {
					// ignore parse errors
				}
		...
		const messageIDs = requestBodies
			.filter(r => r.hasMessageID)
			.map(r => r.messageID)
		if (messageIDs.length === 2) {
			...
			console.log(
				'[test] MessageIDs not present in all requests (may be using undefined)',
			)
		}
	})
})
