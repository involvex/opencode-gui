...
async function connectToWebview(
	cdpUrl: string,
): Promise<{browser: Browser; session: WebviewSession}> {
	...
	while (!webviewPage && attempts < maxAttempts) {
		...
		if (!webviewPage) {
			attempts++
			console.log(
				`[fixture] Waiting for webview to appear (attempt ${attempts}/${maxAttempts})...`,
			)
...
export async function setupExtensionTests(options?: {
	...
	const workspaceRoot =
		options?.workspaceRoot || path.join(process.cwd(), 'tests', 'sandbox')
...
export function getExtensionContext(): ExtensionTestContext {
	if (!sharedContext) {
		throw new Error(
			'Extension test context not initialized. Call setupExtensionTests first.',
		)
	}
	return sharedContext
}
...
export function useExtensionFixture(
	options?: Parameters<typeof setupExtensionTests>[0],
) {
	beforeAll(async () => {
