...
export class CallHandle<TArgs extends any[] = any[], TReturn = any> {
	...
	async reject(error: any): Promise<void> {
		...
		if (this.record.underlyingPromiseReady) {
			...
			this.record.underlyingPromise = Promise.resolve(
				undefined as unknown as TReturn,
			)
			this.record.underlyingPromiseReady.resolve()
		}
		this.record.enterDeferred.resolve()
...
export class GatekeeperHarness<TApis extends Record<string, any> = {}> {
	...
	): GatekeeperHarness<TApis & Record<TName, TApi>> &
		HarnessApis<TApis & Record<TName, TApi>> {
		if (this.instances.size > 0) {
			throw new Error(
				'Cannot add factories after instances have been built. Call add() before raiseAllGates() or lowerAllGates().',
			)
		}
				...
				if (typeof value === 'function') {
					return async (...args: any[]) => {
						...
						if (!record.underlyingPromise) {
							record.underlyingPromise = Promise.resolve(
